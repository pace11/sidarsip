                                        ...
                                        <form id="formAdd" enctype="multipart/form-data">
                                            ...
                                            <div class="modal-body">
                                                <div class="form-group">
                                                    <label for="remark">Institusi</label>
                                                    <input class="form-control" name="name" placeholder="Nama Institusi..." required />
                                                </div>
                                                <div class="form-group">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <table class="example table table-responsive-sm table-bordered table-striped table-sm">
                                        ...
                                        <?php
                                        $no = 1;
                                        $q = mysqli_query($conn, "SELECT * FROM institutions ORDER BY updated_at DESC");
                                        while($data=mysqli_fetch_array($q)){ ?>
                                            <tr>
                                                <td><?= $no ?></td>
                                                <td><?= !empty($data['name']) ? $data['name'] : '-' ?></td>
                                                <td><?= !empty($data['level']) ? strtoupper($data['level']) : '-' ?></td>
                                                <td>
        ...
        <div class="modal-dialog" role="document">
            ...
            <form id="formEdit" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="remark">Institusi</label>
                        <input class="form-control" name="name" id="modal_name" placeholder="Nama Institusi..." required></input>
                    </div>

                    <div class="form-group">
                        <label for="name">Jenjang</label>
                        <select class="form-control" name="level" id="modal_level">
                            <option value="" disabled selected hidden>-- pilih salah satu --</option>
                            <option value="tk">TK</option>
                            <option value="sd">SD</option>
                            <option value="smp">SMP</option>
...
$(function () {
    ...
    $('#formAdd').on('submit', function(e) {
        ...
        $.ajax({
            url: 'config/institusi/add_pro.php',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $('#formEdit').on('submit', function(e) {
        ...
        $.ajax({
            url: 'config/institusi/edit_pro.php',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $('#formDelete').on('submit', function(e) {
        ...
        $.ajax({
            url: 'config/institusi/delete_pro.php',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
